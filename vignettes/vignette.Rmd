...
---
# Preliminaries
`r Biocpkg("CONFESS")` is a customized cell detection and signal estimation model for images coming from the Fluidigm C1 system. Applied to the HeLa `r Biocexptpkg("fucci")` dataset, our method estimated the cell cycle phase of hundreds of samples from their fluorescence signals and enabled us to study the spatio-temporal dynamics of the HeLa cell cycle.
...
## Data pre-processing
The sample set of 14 raw C01 images are available in the `r Biocexptpkg("fucci")` package on Bioconductor. Alternatively, the complete set of 378 images can be downloaded [here](http://single-cell.clst.riken.jp/bluk_download/cell_images/). They include images for each of the following sets: 
...
In this example, we read text-converted files available in the ```fucci``` dataset.
```{r,eval=FALSE}
library(fucci)
